...
public final class TitleScreenReviewCommentsGamePuppet {
 ...
 public static void run(SFMGamePuppetHelper p){
  ...
  p.applyReviewCommentFixtureCommand("style-picker");p.setColorInputHex("#FFFF33AA",false);p.capture("review-comments-colour-picker",cap("The reusable Theme Settings colour input edits the #problem underline through a typed callback."));
  p.confirmColorInput();p.waitTicks(6);p.applyReviewCommentFixtureCommand("styles");p.capture("review-comments-colour-applied",cap("The #problem rule now displays the applied ARGB value without changing comment semantics."));
