...
/*
        ...
        for (int i = 0; i < 5; i++) {
            ...
            helper.assertTrue(tank != null, "Water tank block entity should exist at position " + i, helper.getTick());
            helper.assertTrue(tank.isActive(), "Water tank " + i + " should be active when surrounded by water", helper.getTick());
        }
        ...
        for (int i = 0; i < 5; i++) {
            WaterTankBlockEntity tank = helper.getBlockEntity(tankPositions[i], WaterTankBlockEntity.class);
            helper.assertTrue(!tank.isActive(), "Water tank " + i + " should be inactive with only 1 water source", helper.getTick());
        }
        ...
        for (int i = 0; i < 5; i++) {
            WaterTankBlockEntity tank = helper.getBlockEntity(tankPositions[i], WaterTankBlockEntity.class);
            helper.assertTrue(tank.isActive(), "Water tank " + i + " should be active again after restoring water", helper.getTick());
        }
