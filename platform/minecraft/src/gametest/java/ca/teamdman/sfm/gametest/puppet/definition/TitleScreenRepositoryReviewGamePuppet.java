...
public final class TitleScreenRepositoryReviewGamePuppet {
    ...
    public static void run(SFMGamePuppetHelper p) {
        ...
        p.waitTicks(10);
        p.capture("repository-review-search", cap("Search narrows the bundle to one changed source."));
        p.applyRepositoryReviewCommand("search:");
        p.applyRepositoryReviewCommand("select:0");
        p.applyRepositoryReviewCommand("line:after:0");
        p.waitTicks(10);
        p.capture("repository-review-selection", cap("The reviewer explicitly selects after line 1, shown with its UTF-8 byte range and source highlight."));
