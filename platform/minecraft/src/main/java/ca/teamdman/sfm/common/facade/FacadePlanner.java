...
public class FacadePlanner {
    public static @Nullable IFacadePlan getFacadePlan(
        ...
        // preconditions
        BlockPos hitPos = msg.pos();
        if (!level.isBlockLoaded(hitPos)) return null;
        IBlockState hitBlockState = level.getBlockState(hitPos);
        ...
        // Apply facade
        IBlockState renderBlockState = renderBlock.getStateForPlacement(
                player.world, msg.pos(), msg.facing(), 0, 0, 0, paintItem.getMetadata(msg.paintStack().getItemDamage()), player, msg.hand()
        );

        FacadeTransparency facadeTransparency = renderBlockState.isTranslucent()
            ...
            case NETWORK_GLOBAL_SAME_PAINT -> {
                if (level.getTileEntity(hitPos) instanceof IFacadeBlockEntity startFacadeBlockEntity) {
                    ...
                    yield CableNetwork.discoverCables(level, hitPos)
                            ...
                            .filter(cablePos -> {
                                if (
                                        level.getTileEntity(cablePos) instanceof IFacadeBlockEntity otherFacadeBlockEntity
                                                && otherFacadeBlockEntity.getClass().equals(
                                                existingFacadeBlockEntityClass)
                                ) {
                            ...
                            (current, next, results) -> {
                                ...
                                SFMBlockPosUtils.get3DNeighboursIncludingKittyCorner(current)
                                        .filter(neighbour -> {
                                            ...
                                            if (
                                                    level.getTileEntity(neighbour) instanceof IFacadeBlockEntity otherCableFacadeBlockEntity
                                                            && otherCableFacadeBlockEntity.getClass().equals(
                                                            existingFacadeBlockEntityClass)
                                            ) {
