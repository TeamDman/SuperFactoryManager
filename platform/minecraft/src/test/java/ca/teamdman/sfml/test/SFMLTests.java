...
public class SFMLTests {
    ...
    public void badTimerIntervalCheckingConfig() {
        var min = SFMConfig.server.timerTriggerMinimumIntervalInTicks;
        var template = """
                    name "hello world"
    ...
    public void badLabelLength() {
        var template = """
                ...
                """;
        String program = template.replace("X", new String(new char[257]).replace("\0", "a"));
        assertCompileErrorsPresent(
                program,
                new CompileErrors(
    ...
    public void forgeTimerIntervalPass() {
        var min = SFMConfig.server.timerTriggerMinimumIntervalInTicksWhenOnlyForgeEnergyIO;
        assertEquals(1, min);
        var input = """
                    name "hello world"
...
""";
        ...
        String colouredInput = colouredLines.stream().map(ITextComponent::getFormattedText).collect(Collectors.joining("\n"));

        assertEquals(rawInput, colouredInput);

        // newlines should not be present
        // instead, each line should be its own component
        assertFalse(colouredLines.stream().anyMatch(x -> x.getFormattedText().contains("\n")));

        assertEquals(lines.length, colouredLines.size());
        for (int i = 0; i < lines.length; i++) {
            assertEquals(lines[i], colouredLines.get(i).getFormattedText());
        }
    }
...
""";
        ...
        for (int i = 0; i < rawRawLines.length; i++) {
            ...
            for (int j = 0; j < lines.length; j++) {
                assertEquals(lines[j], colouredLines.get(j).getFormattedText());
            }
        }
    }
