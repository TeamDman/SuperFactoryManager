...
public class ExtendedButton extends Button {
    ...
    public void renderWidget(int mouseX, int mouseY, float partialTick) {
        ...
        Tools.blitSprite(SPRITES.get(this.active, this.isHoveredOrFocused()), this.getX(), this.getY(), this.getWidth(),
                this.getHeight());

        String ellipsize = Tools.ellipsize(this.getMessage().getFormattedText(), this.width - 6);// Remove 6 pixels so
                                                                                                 // that the text is
                                                                                                 // always contained
                                                                                                 // within the button's
                                                                                                 // borders
        Tools.drawCenteredString(ellipsize, this.getX() + this.width / 2, this.getY() + (this.height - 8) / 2,
                getFGColor());
    }
