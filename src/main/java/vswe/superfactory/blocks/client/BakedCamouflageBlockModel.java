...
public class BakedCamouflageBlockModel implements IBakedModel {
    ...
    private java.util.function.Function<ResourceLocation, TextureAtlasSprite> bakedTextureGetter;
    ...
    public BakedCamouflageBlockModel(IModelState modelState, VertexFormat format,
    ...
    public List<BakedQuad> getQuads(IBlockState state, EnumFacing side, long rand) {
        if (state instanceof IExtendedBlockState) {
            ...
            if (obj != null) {
                ...
                BlockRendererDispatcher dispatcher = FMLClientHandler.instance().getClient()
                ...
                if (tileEntity instanceof TileEntityCluster) {
                    ...
                    camouflage = TileEntityCluster.getTileEntity(TileEntityCamouflage.class,
                            FMLClientHandler.instance().getWorldClient(), pos);
                    ...
                    if (camouflage == null) {
                        ...
                        try {
                            clusterModel = ModelLoaderRegistry
                                    .getModel(BlockCableCluster.isAdvanced(cluster.getBlockMetadata()) ?
                                            CamouflageBlockModel.MODEL_CLUSTER_ADV :
                                            CamouflageBlockModel.MODEL_CLUSTER);
                        } catch (Exception ignored) {}

                        if (clusterModel != null) {
                            return clusterModel.bake(modelState, format, bakedTextureGetter).getQuads(state, side,
                                    rand);
                        }
                    }
                ...
                return new AssembledBakedModel(camouflage, pos, blockState, modelShapes, cluster, format)
    ...
    public class AssembledBakedModel implements IBakedModel {
        ...
        public AssembledBakedModel(TileEntityCamouflage camouflage, BlockPos pos, IExtendedBlockState blockState,
                                   BlockModelShapes modelShapes, TileEntityCluster cluster, VertexFormat format) {
            this.format = format;
            ...
            if (camouflage != null && quads.isEmpty()) {
                ...
                for (EnumFacing facing : EnumFacing.values()) {
                    ...
                    Block insideBlock = Block
                            .getBlockById(camouflage.getId(facing.getIndex() + EnumFacing.values().length));
        ...
        public void generateQuads(Block block, BlockPos pos, IExtendedBlockState blockState,
                                  BlockModelShapes modelShapes, TileEntityCamouflage camouflage, EnumFacing facing,
                                  boolean inside, TileEntityCluster cluster) {
            if (block instanceof BlockAir || block instanceof BlockCableCamouflages ||
                    block instanceof BlockCableCluster) {
                if (cluster == null) {
                    TileEntityCamouflage.CamouflageType camoType = (TileEntityCamouflage.CamouflageType) blockState
                            .getValue(BlockCableCamouflages.CAMO_TYPE);
                    quads.add(getTransformedQuad(blockState, pos, blockState.getBlock(), facing, camoType.getIcon(),
                            camoType == TileEntityCamouflage.CamouflageType.NORMAL && !inside ? normalSprite :
                                    camoType == TileEntityCamouflage.CamouflageType.INSIDE ? insideSprite :
                                            transformSprite,
                            inside, 0));
                } else {
                    BlockCableCluster blockCluster = (BlockCableCluster) cluster.getBlockType();
                    ...
                    boolean isFacingFront = ((camouflage != null && camouflage.getCamouflageType().useSpecialShape() &&
                            inside) || (camouflage == null && inside)) ? (facing == clusterFacing.getOpposite()) :
                                    (facing == clusterFacing);
                    boolean isAdvanced = BlockCableCluster.isAdvanced(clusterMeta);

                    String resource = (isAdvanced ?
                            (isFacingFront ? CamouflageBlockModel.CL_ADV_FRONT : CamouflageBlockModel.CL_ADV_SIDE) :
                            isFacingFront ? CamouflageBlockModel.CL_FRONT : CamouflageBlockModel.CL_SIDE).toString();
                    TextureAtlasSprite texture = isAdvanced ? (isFacingFront ? clusterFrontAdv : clusterSideAdv) :
                            isFacingFront ? clusterFront : clusterSide;

                    quads.add(getTransformedQuad(blockState, pos, blockState.getBlock(), facing, resource, texture,
                            inside, 0));
                }
            } else {
                IBlockState camoState = block.getStateFromMeta(
                        camouflage.getMeta(facing.getIndex() + (inside ? EnumFacing.values().length : 0)));
                ...
                for (BakedQuad quad : bakedQuads) {
                    if (quad.getFace() == facing) {
                        if (camouflage.getCamouflageType().useDoubleRendering()) {
                            quad = reBakeQuadForBlock(quad, blockState, pos, blockState.getBlock(), facing,
                                    quad.getSprite(), inside, camouflage.rotate);
                        }
        ...
        private BakedQuad getTransformedQuad(IExtendedBlockState blockState, BlockPos pos, Block block,
                                             EnumFacing facing, String resource, TextureAtlasSprite sprite,
                                             boolean inside, int rotation) {
            AxisAlignedBB alignedBB = block.getBoundingBox(blockState, FMLClientHandler.instance().getWorldClient(),
                    pos);
            ...
            ModelRotation modelRotation = inside ? ModelRotation.getModelRotation(180, rotation * 90) :
                    ModelRotation.getModelRotation(0, rotation * 90);
            return bakery.makeBakedQuad(new Vector3f(minX, minY, minZ), new Vector3f(maxX, maxY, maxZ),
                    new BlockPartFace(facing, -1, resource == null ? "" : resource, faceUV), sprite,
                    inside ? facing.getOpposite() : facing, modelRotation, null, false, true);
        }

        private BakedQuad reBakeQuadForBlock(BakedQuad original, IExtendedBlockState blockState, BlockPos pos,
                                             Block block, EnumFacing facing, TextureAtlasSprite sprite, boolean inside,
                                             int rotation) {
            BakedQuad transformedQuad = getTransformedQuad(blockState, pos, block, facing, null, sprite, inside,
                    rotation);
            ...
            for (int i = 0; i < 4; i++) {
                ...
                for (int j = 0; j < 3; j++) {
                    d[j] = (Float.intBitsToFloat(faceData[storeIndex + j]) * 16) -
                            (Float.intBitsToFloat(transformedFaceData[storeIndex + j]) * 16);
                }
