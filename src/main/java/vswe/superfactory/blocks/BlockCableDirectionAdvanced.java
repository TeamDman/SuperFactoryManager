...
public abstract class BlockCableDirectionAdvanced extends BlockContainer {
    ...
    public BlockCableDirectionAdvanced() {
        ...
        this.setDefaultState(
                this.blockState.getBaseState().withProperty(FACING, EnumFacing.NORTH).withProperty(ADVANCED, false));
    }
    ...
    public int getMetaFromState(IBlockState state) {
        return addAdvancedMeta(((EnumFacing) state.getValue(FACING)).getIndex(),
                ((Boolean) state.getValue(ADVANCED)) ? 8 : 0);
    }
    ...
    public void onBlockPlacedBy(World world, BlockPos pos, IBlockState state, EntityLivingBase entity, ItemStack item) {
        int meta = addAdvancedMeta(EnumFacing.getDirectionFromEntityLiving(pos, entity).getIndex(),
                item.getItemDamage());
