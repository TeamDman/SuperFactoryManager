...
public class ConnectionBlock implements IContainerSelection {
    ...
    public static boolean isOfType(EnumSet<ConnectionBlockType> types, ConnectionBlockType type) {
        return type == null || types.contains(type) || (type == ConnectionBlockType.NODE &&
                (types.contains(ConnectionBlockType.RECEIVER) || types.contains(ConnectionBlockType.EMITTER)));
    }
    ...
    public String getDescription(GuiManager gui) {
        ...
        str += "\n" + Localization.X + ": " + tileEntity.getPos().getX() + " " + Localization.Y + ": " +
                tileEntity.getPos().getY() + " " + Localization.Z + ": " + tileEntity.getPos().getZ();
    ...
    public int getDistance(TileEntityManager manager) {
        return (int) Math.round(Math.sqrt(manager.getDistanceSq(tileEntity.getPos().getX() + 0.5,
                tileEntity.getPos().getY() + 0.5, tileEntity.getPos().getZ() + 0.5)));
    }
    ...
    public boolean isPartOfVariable(Variable variable) {
        return variable.isValid() && ((ComponentMenuContainer) variable.getDeclaration().getMenus().get(2))
                .getSelectedInventories().contains(id);
    }
