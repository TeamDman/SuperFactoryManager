...
public class TileEntityBreaker extends TileEntityClusterElement implements IInventory, IPacketBlock {
    ...
    private List<ItemStack> placeItem(ItemStack itemstack) {
        ...
        if (!itemstack.isEmpty() && itemstack.getItem() != null && itemstack.getCount() > 0) {
            ...
            EntityPlayerMP player = FakePlayerFactory.get((WorldServer) world,
            ...
            player.prevRotationYaw = player.rotationPitch = direction == EnumFacing.UP ? 90 :
                    direction == EnumFacing.DOWN ? -90 : 0;
            ...
            try {
                ...
                if (result.getType().equals(EnumActionResult.PASS) &&
                        ItemStack.areItemStacksEqual(result.getResult(), itemstack)) {
                    ...
                    player.interactionManager.processRightClickBlock(player, world, itemstack, EnumHand.MAIN_HAND,
                            new BlockPos(x, y, z), direction, hitX, hitY, hitZ);
            ...
            if (!itemStack.isEmpty()) {
                ...
                if (!broken) {
                    for (int i = 0; i < inventoryCache.size(); i++) {
                        ...
                        if (!copyStack.isEmpty() && newStack.isItemEqual(copyStack) &&
    ...
    private void keepClientDataUpdated() {
        ...
        if (distance > Math.pow(PacketHandler.BLOCK_UPDATE_RANGE, 2)) {
            ...
            if (!hasUpdatedData && distance < Math.pow(PacketHandler.BLOCK_UPDATE_RANGE - UPDATE_BUFFER_DISTANCE, 2)) {
                hasUpdatedData = true;
    ...
    public void writeData(DataWriter dw, EntityPlayer player, boolean onServer, int id) {
        if (onServer) {
            if (placeDirection == null)
                placeDirection = BlockCableBreaker.getSide(getBlockMetadata()); // might be a cheap fix, but seams to
                                                                                // some kind of sync bug between threads
                                                                                // or something
    ...
    public void markDirty() {
        ...
        if (inventory != null && !broken) {
            ...
            for (int i = 0; i < inventory.size(); i++) {
                ...
                if (!itemStackCopy.isEmpty() && (itemStack.isEmpty() ||
                        Item.getIdFromItem(itemStack.getItem()) != Item.getIdFromItem(itemStackCopy.getItem()) ||
                        itemStack.getItemDamage() != itemStackCopy.getItemDamage() ||
                        !ItemStack.areItemStackTagsEqual(itemStack, itemStackCopy) ||
                        itemStack.getCount() < itemStackCopy.getCount())) {
                    match = false;
                    break;
                }
            }
    ...
    private boolean canBreakBlock(IBlockState state, Block block, BlockPos pos) {
        return block != null && Block.getIdFromBlock(block) != Block.getIdFromBlock(Blocks.BEDROCK) &&
                block.getBlockHardness(state, world, pos) >= 0;
    }
