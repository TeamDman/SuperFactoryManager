...
public class TileEntityOutput extends TileEntityClusterElement implements IPacketBlock, IRedstoneNode {
        ...
        for (int i = 0; i < EnumFacing.values().length; i++) {
            if (sides.isSideRequired(i)) {
                ...
                if (pulse.shouldEmitPulse()) {
                    PulseTimer timer = new PulseTimer(oldStrength, oldStrong, pulse.getPulseTime() + 1); // add one to
                                                                                                         // counter the
                                                                                                         // first tick
                                                                                                         // (which is
                                                                                                         // the same
    ...
    private void notifyUpdate(BlockPos pos, boolean spread) {
        if (world.getBlockState(pos).getBlock() != null) {
            ...
            world.notifyBlockUpdate(pos, SFMBlocks.CABLE_OUTPUT.getDefaultState(),
                    SFMBlocks.CABLE_OUTPUT.getDefaultState(), 3);
    ...
    public void readContentFromNBT(NBTTagCompound nbtTagCompound) {
        ...
        for (int i = 0; i < sidesTag.tagCount(); i++) {
            ...
            for (int j = 0; j < pulsesTag.tagCount(); j++) {
                ...
                timers.add(new PulseTimer(pulseTag.getByte(NBT_STRENGTH), pulseTag.getBoolean(NBT_STRONG),
                        pulseTag.getShort(NBT_TICK)));
            }
        }
    }
    ...
    protected EnumSet<ClusterMethodRegistration> getRegistrations() {
        return EnumSet.of(ClusterMethodRegistration.CAN_CONNECT_REDSTONE,
                ClusterMethodRegistration.SHOULD_CHECK_WEAK_POWER, ClusterMethodRegistration.IS_PROVIDING_WEAK_POWER,
