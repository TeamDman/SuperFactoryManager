...
public class TileEntitySignUpdater extends TileEntityClusterElement {
    ...
    public void updateSign(ComponentMenuSignText menu) {
        ...
        TileEntity te = world.getTileEntity(new BlockPos(getPos().getX() + direction.getXOffset(),
                getPos().getY() + direction.getYOffset(), getPos().getZ() + direction.getZOffset()));
        if (te instanceof TileEntitySign) {
            ...
            if (updated) {
                sign.markDirty();
                world.notifyBlockUpdate(sign.getPos(), getWorld().getBlockState(sign.getPos()),
                        getWorld().getBlockState(sign.getPos()), 3);
            }
        }
    }
}
