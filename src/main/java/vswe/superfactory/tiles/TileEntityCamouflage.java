...
public class TileEntityCamouflage extends TileEntityClusterElement implements IPacketBlock {
    ...
    // public boolean addBlockEffect(BlockCamouflageBase camoBlock, IBlockState state, World world, EnumFacing sideHit,
    ...
    // double x = (double) getPos().getX() + rand.nextDouble() * (axisalignedbb.maxX - axisalignedbb.minX - (double) (f
    // * 2.0F)) + (double) f + axisalignedbb.minX;
    // double y = (double) getPos().getY() + rand.nextDouble() * (axisalignedbb.maxY - axisalignedbb.minY - (double) (f
    // * 2.0F)) + (double) f + axisalignedbb.minY;
    // double z = (double) getPos().getZ() + rand.nextDouble() * (axisalignedbb.maxZ - axisalignedbb.minZ - (double) (f
    // * 2.0F)) + (double) f + axisalignedbb.minZ;
    //
    // switch (sideHit) {
    // case DOWN:
    ...
    //
    //
    // // effectRenderer.addEffect((new EntityDiggingFX.Factory().getEntityFX(0, this.worldObj, x, y, z, 0.0D, 0.0D,
    // 0.0D, Block.getIdFromBlock(camoBlock))).multiplyVelocity(0.2F).multipleParticleScaleBy(0.6F));
    // return true;
    // }
    // }
    ...
    public AxisAlignedBB getBlockBounds() {
        return new AxisAlignedBB(bounds[0] / 32D, bounds[2] / 32D, bounds[4] / 32D, bounds[1] / 32D, bounds[3] / 32D,
                bounds[5] / 32D);
    }
        ...
        if (onServer) {
            ...
            if (getCamouflageType().useSpecialShape()) {
                ...
                for (int bound : bounds) {
                    // This is done since 0 and 32 are the most common values and the final bit would only be used by 32
                    // anyways
                    // 0 -> 01
                    // 32 -> 11
                    // 1 to 31 -> bin(bound) << 1
    ...
    public void readData(DataReader dr, EntityPlayer player, boolean onServer, int id) {
        if (onServer) {
            ...
            world.notifyBlockUpdate(getPos(), getWorld().getBlockState(getPos()), getWorld().getBlockState(getPos()),
                    3);
        }
    }
    ...
    protected void keepClientDataUpdated() {
        double distance = Minecraft.getMinecraft().player.getDistanceSq(getPos().getX() + 0.5, getPos().getY() + 0.5,
                getPos().getZ() + 0.5);

        if (distance > Math.pow(PacketHandler.BLOCK_UPDATE_RANGE, 2)) {
            hasClientUpdatedData = false;
        } else if (!hasClientUpdatedData &&
    ...
    public enum CamouflageType implements IStringSerializable {
        ...
        NORMAL("cable_camouflage", "cable_camouflage", false, false),
        INSIDE("cable_camouflage_inside", "cable_camouflage_inside", true, false),
        SHAPE("cable_camouflage_shape", "cable_camouflage_shape", true, true);
