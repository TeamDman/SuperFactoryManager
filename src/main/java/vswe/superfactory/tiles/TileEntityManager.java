...
public class TileEntityManager extends TileEntity implements ITileEntityInterface, ITickable {
            ...
            if (timer >= 20) {
                ...
                for (FlowComponent item : items) {
                    ...
                    if (item.getType() == ComponentType.TRIGGER) {
                        ComponentMenuInterval componentMenuInterval = (ComponentMenuInterval) item.getMenus()
                                .get(TriggerHelper.TRIGGER_INTERVAL_ID);
    ...
    public void updateInventories() {
        ...
        for (int i = 0; i < oldCoordinates.length; i++) {
            ...
            oldCoordinates[i] = new WorldCoordinate(inventory.getPos().getX(), inventory.getPos().getY(),
                    inventory.getPos().getZ());
                ...
                for (int y = -1; y <= 1; y++) {
                    for (int z = -1; z <= 1; z++) {
                        if (Math.abs(x) + Math.abs(y) + Math.abs(z) == 1) {
                            WorldCoordinate target = new WorldCoordinate(element.getX() + x, element.getY() + y,
                                    element.getZ() + z, element.getDepth() + 1);

                            if (!visited.contains(target) &&
                                    (Settings.isLimitless(this) || inventories.size() < MAX_CONNECTED_INVENTORIES)) {
                                visited.add(target);
                                TileEntity te = world
                                        .getTileEntity(new BlockPos(target.getX(), target.getY(), target.getZ()));

                                if (te != null) {
                                    if (te instanceof TileEntityCluster) {
                                        for (TileEntityClusterElement tileEntityClusterElement : ((TileEntityCluster) te)
                                                .getElements()) {
                                ...
                                if ((Settings.isLimitless(this) || element.getDepth() < MAX_CABLE_LENGTH) &&
                                        CableNetwork.isCable(world, pos)) {
                                    queue.add(target);
                                }
                            }
                        }
        ...
        if (firstInventoryUpdate) {
            if (initialInventoryPositions.size() > 0) {
                ...
                for (int i = 0; i < oldCoordinates.length; i++) {
                    ...
                    oldCoordinates[i] = new WorldCoordinate(inventoryPos.getX(), inventoryPos.getY(),
                            inventoryPos.getZ());
        ...
        for (WorldCoordinate oldCoordinate : oldCoordinates) {
            if (oldCoordinate.getTileEntity() instanceof ISystemListener) {
                boolean found = false;
                for (ConnectionBlock inventory : inventories) {
                    if (oldCoordinate.getX() == inventory.getTileEntity().getPos().getX() &&
                            oldCoordinate.getY() == inventory.getTileEntity().getPos().getY() &&
                            oldCoordinate.getZ() == inventory.getTileEntity().getPos().getZ()) {
                        found = true;
                        break;
                    }
                }
    ...
    private List<Integer> getNewSelection(WorldCoordinate[] oldCoordinates, List<Integer> oldSelection,
            ...
            if (hasVariables && selection >= 0 && selection < 16) {
                ...
                if (selection >= 0 && selection < oldCoordinates.length) {
                    ...
                    for (int j = 0; j < inventories.size(); j++) {
                        ...
                        if (coordinate.getX() == inventory.getPos().getX() &&
                                coordinate.getY() == inventory.getPos().getY() &&
                                coordinate.getZ() == inventory.getPos().getZ() &&
                                inventory.getClass().equals(coordinate.getTileEntity().getClass())) {
        ...
        for (int i = 0; i < flowComponent.getConnectionSet().getConnections().length; i++) {
            ...
            if (hasConnection) {
                ...
                for (int j = 0; j < length; j++) {
                    connection.getNodes().add(new Point(dr.readData(DataBitHelper.FLOW_CONTROL_X),
                            dr.readData(DataBitHelper.FLOW_CONTROL_Y)));
                }
            }
        }
    ...
    public void writeContentToNBT(NBTTagCompound nbtTagCompound, boolean pickup) {
        nbtTagCompound.setByte(SuperFactoryManager.NBT_PROTOCOL_VERSION,
                SuperFactoryManager.NBT_CURRENT_PROTOCOL_VERSION);
                ...
                for (int i = 0; i < item.getConnectionSet().getConnections().length; i++) {
                    ...
                    if (connection != null) {
                        if (connection.getComponentId() < 0 || connection.getComponentId() >= items.size()) {
                            ...
                            if (otherConnection == null || otherConnection.getComponentId() < 0 ||
                                    otherConnection.getComponentId() >= items.size()) {
