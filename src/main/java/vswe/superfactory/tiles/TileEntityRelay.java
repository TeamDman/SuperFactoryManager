...
public class TileEntityRelay extends TileEntityClusterElement
        ...
        if (world != null) {
            ...
            if (entities != null) {
                ...
                for (Entity entity : entities) {
                    double distance = entity.getDistanceSq(getPos().getX() + 0.5, getPos().getY() + 0.5,
                            getPos().getZ() + 0.5);
    ...
    private boolean isEntityValid(Entity entity, Class type, int id) {
        return type.isInstance(entity) ||
                (id == 0 && ((entity instanceof EntityPlayer && allowPlayerInteraction((EntityPlayer) entity)) ||
    ...
    public void readContentFromNBT(NBTTagCompound nbtTagCompound) {
        ...
        if (nbtTagCompound.hasKey(NBT_OWNER)) {
            ...
            for (int i = 0; i < permissionTags.tagCount(); i++) {
                ...
                UserPermission permission = new UserPermission(UUID.fromString(permissionTag.getString(NBT_UUID)),
                        permissionTag.getString(NBT_NAME));
    ...
    public void readAllData(DataReader dr, EntityPlayer player) {
        ...
        for (int i = 0; i < length; i++) {
            UserPermission permission = new UserPermission(UUID.fromString(dr.readString(DataBitHelper.UUID_LENGTH)),
                    dr.readString(DataBitHelper.NAME_LENGTH));
    ...
    public void readUpdatedData(DataReader dr, EntityPlayer player) {
        ...
        if (userData) {
            ...
            if (added) {
                ...
                UserPermission permission = new UserPermission(UUID.fromString(UUIDS),
                        dr.readString(DataBitHelper.NAME_LENGTH));
                ...
                if (permissions.size() < TileEntityRelay.PERMISSION_MAX_LENGTH &&
                        (world.isRemote || permission.getUserId().equals(userId))) {
                    permissions.add(permission);
                }
            } else {
        ...
        if (permissions.size() > container.oldPermissions.size()) {
            ...
            for (int i = 0; i < container.oldPermissions.size(); i++) {
                if (i >= permissions.size() ||
                        !permissions.get(i).getUserId().equals(container.oldPermissions.get(i).getUserId())) {
                    DataWriter dw = PacketHandler.getWriterForUpdate(container);
                    dw.writeBoolean(true); // user data
