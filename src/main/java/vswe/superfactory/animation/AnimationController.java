...
public class AnimationController {
    ...
    private void addComponent(FlowComponent component, Map<Integer, Integer> ids,
                              Map<Integer, List<FlowComponent>> groups) {
                ...
                if (target.getType() != ComponentType.GROUP || groupNodes.get(target.getId()) == null) {
                    ...
                    if (manager.getCurrentlyConnecting() != null) {
                        ...
                        if (distance < 1) {
                            ...
                            manager.getFlowItems().get(connection.getComponentId())
                        ...
                        for (int i = 0; i < blueprint.getConnectionSet().getConnections().length; i++) {
                            if (target.getConnection(i) == null) {
                                ...
                                if (connection != null && connection.getComponentId() < virtualId &&
                                    ...
                                    if (location != null) {
                                        ...
                                        targetConnectionX = targetConnectionLocation[0] +
                                                targetConnectionLocation[3] / 2;
                                        targetConnectionY = targetConnectionLocation[1] +
                                                targetConnectionLocation[4] / 2;
                                        return true;
                                    } else {
                    ...
                    if (blueprintNode == null) {
                        if (nodesBlueprint.size() == 0) {
                            ...
                            if (nodesConnection.getNodes().size() == 0) {
                                ...
                                nodesConnection.addAndSelectNode(nodesConnection.getNodes().get(0).getX(),
                                        nodesConnection.getNodes().get(0).getY(), 0);
                            }
                    ...
                    for (int i = 0; i < target.getConnectionSet().getConnections().length; i++) {
                        if (target.getConnection(i) != null) {
                            ...
                            Connection connection = reverse ?
                                    other.getConnection(target.getConnection(i).getConnectionId()) :
                                    blueprint.getConnection(i);
                            Connection targetConnection = reverse ?
                                    otherTarget.getConnection(target.getConnection(i).getConnectionId()) :
                            ...
                            if (targetConnection.getNodes().isEmpty() && !connection.getNodes().isEmpty()) {
                                ...
                                int[] location = reverse ?
                                        other.getConnectionLocationFromId(target.getConnection(i).getConnectionId()) :
