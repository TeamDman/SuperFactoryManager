...
public class ItemMemoryDisk extends Item {
    ...
    public static boolean validateNBT(ItemStack stack) {
        return stack.hasTagCompound() &&
                (stack.getTagCompound().getString("id").equals("superfactorymanager:manager") ||
                        stack.getTagCompound().getString("id").equals("rfmanager"));
        // stack.setTagCompound(null);
    }
            ...
            for (int variablesTag = 0; variablesTag < components.tagCount(); ++variablesTag) {
                ...
                for (int i = 0; i < connections.tagCount(); ++i) {
                    ...
                    if (connection.hasKey("ConnectionComponent")) {
                        if (version < 9) {
                            connection.setByte("ConnectionComponent",
                                    (byte) (connection.getByte("ConnectionComponent") + currentFlow));
                        } else {
                            connection.setShort("ConnectionComponent",
