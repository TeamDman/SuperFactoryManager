...
public class GuiRelay extends GuiBase {
    ...
    protected void drawGuiContainerBackgroundLayer(float f, int mX, int mY) {
        ...
        if (!relay.doesListRequireOp() || isOp(player, true)) {
            ...
            for (int i = start; i <= getEndId(); i++) {
                ...
                drawTexture(x, y, srcXMenu * LIST_MENU_WIDTH, ySize + srcYMenu * LIST_MENU_HEIGHT, LIST_MENU_WIDTH,
                        LIST_MENU_HEIGHT);

                for (int j = 0; j < 2; j++) {
                    renderInfoBox(j, permission, x + LIST_MENU_WIDTH - (1 + j) * (INFO_SIZE + INFO_MARGIN_X),
                            y + INFO_MARGIN_Y);
                }
            ...
            if (getPageCount() > 1) {
                drawCenteredString(Localization.PAGE.toString() + " " + (currentPage + 1) + " " +
                        Localization.OF.toString() + " " + getPageCount(), LIST_POS_X, PAGE_Y, 0.6F, LIST_MENU_WIDTH,
                        0x404040);
                ...
                for (int i = 0; i < 2; i++) {
                    ...
                    drawTexture(
                            x, y, srcXButton * PAGE_BUTTON_SIZE, ySize + LIST_MENU_HEIGHT * 2 + INFO_SIZE +
                                    BUTTON_HEIGHT * 3 + srcYButton * PAGE_BUTTON_SIZE,
                            PAGE_BUTTON_SIZE, PAGE_BUTTON_SIZE);
                }
            }
        } else {
            drawCenteredString(Localization.NO_ACCESS.toString(), LIST_POS_X, LIST_POS_Y + NO_ACCESS_TEXT_Y, 1F,
                    LIST_MENU_WIDTH, 0x804040);
        }

        UserPermission info = relay.doesListRequireOp() && !isOp(player, true) ? player : selected;
        if (info != null) {
            drawString(info.getUserName(), INFO_BOX_POS_X + INFO_BOX_NAME_X, INFO_BOX_POS_Y + INFO_BOX_NAME_Y, 0.7F,
                    0x404040);
            for (int i = 0; i < 2; i++) {
            ...
            drawSplitString(str, INFO_BOX_POS_X + INFO_BOX_FULL_TEXT_X, INFO_BOX_POS_Y + INFO_BOX_FULL_TEXT_Y,
                    INFO_BOX_FULL_TEXT_W, 0.7F, 0x404040);
        }

        for (Button button : buttons) {
            if (button.isVisible()) {
                int textureId = button.isEnabled() ?
                        CollisionHelper.inBounds(button.x, button.y, BUTTON_WIDTH, BUTTON_HEIGHT, mX, mY) ? 2 : 1 : 0;

                drawTexture(button.x, button.y, 0, ySize + LIST_MENU_HEIGHT * 2 + INFO_SIZE + BUTTON_HEIGHT * textureId,
                        BUTTON_WIDTH, BUTTON_HEIGHT);
                drawCenteredString(button.name.toString(), button.x, button.y + BUTTON_TEXT_Y, 0.4F, BUTTON_WIDTH,
    ...
    private boolean isOwner(UserPermission permission, boolean viewer) {
        return (permission != null && permission.getUserId().equals(relay.getOwner())) ||
                (viewer && getUserId().equals(relay.getOwner()));
    }
    ...
    private String getUserName() {
        return Utils.stripControlCodes(Minecraft.getMinecraft().player.getDisplayNameString());
    }
