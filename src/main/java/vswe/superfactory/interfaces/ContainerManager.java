...
public class ContainerManager extends ContainerBase {
    ...
    public void addListener(IContainerListener listener) {
        ...
        for (ConnectionBlock connection : manager.getConnectedInventories()) {
            oldInventories.add(new WorldCoordinate(connection.getTileEntity().getPos().getX(),
                    connection.getTileEntity().getPos().getY(), connection.getTileEntity().getPos().getZ()));
        }
        ...
        if (oldComponents != null) {
            ...
            if (!hasInventoriesChanged) {
                for (int i = 0; i < oldInventories.size(); i++) {
                    ...
                    if (oldInventories.get(i).equals(new WorldCoordinate(tileEntity.getPos().getX(),
    ...
    public boolean canInteractWith(EntityPlayer entityplayer) {
        return entityplayer.getDistanceSq(manager.getPos().getX(), manager.getPos().getY(), manager.getPos().getZ()) <=
                64;
    }
}
