...
public class GuiManager extends GuiBase {
    ...
    protected void drawGuiContainerBackgroundLayer(float f, int x, int y) {
        ...
        if (useButtons) {
            for (int i = 0; i < manager.buttons.size(); i++) {
                ...
                if (button.isVisible()) {
                    int srcButtonY = CollisionHelper.inBounds(button.getX(), button.getY(),
                            TileEntityManager.BUTTON_SIZE_W, TileEntityManager.BUTTON_SIZE_H, x, y) ? 1 : 0;

                    drawTexture(button.getX(), button.getY(), TileEntityManager.BUTTON_SRC_X,
                            TileEntityManager.BUTTON_SRC_Y + srcButtonY * TileEntityManager.BUTTON_SIZE_H,
                            TileEntityManager.BUTTON_SIZE_W, TileEntityManager.BUTTON_SIZE_H);
                    drawTexture(button.getX() + 1, button.getY() + 1, TileEntityManager.BUTTON_INNER_SRC_X,
                            TileEntityManager.BUTTON_INNER_SRC_Y + i * TileEntityManager.BUTTON_INNER_SIZE_H,
                            TileEntityManager.BUTTON_INNER_SIZE_W, TileEntityManager.BUTTON_INNER_SIZE_H);
                }
            }
        }
        ...
        for (int i = 0; i < manager.getZLevelRenderingList().size(); i++) {
            ...
            if (itemBase.isVisible()) {
                ...
                if (itemBase.isBeingMoved() || CollisionHelper.inBounds(itemBase.getX(), itemBase.getY(),
                        itemBase.getComponentWidth(), itemBase.getComponentHeight(), x, y)) {
        ...
        if (useMouseOver) {
            ...
            if (useButtons) {
                for (TileEntityManager.Button button : manager.buttons) {
                    if (button.isVisible() && CollisionHelper.inBounds(button.getX(), button.getY(),
                            TileEntityManager.BUTTON_SIZE_W, TileEntityManager.BUTTON_SIZE_H, x, y)) {
    ...
    private void onClickButtonCheck(int x, int y, boolean release) {
        for (int i = 0; i < manager.buttons.size(); i++) {
            ...
            if (guiButton.isVisible() && CollisionHelper.inBounds(guiButton.getX(), guiButton.getY(),
                    TileEntityManager.BUTTON_SIZE_W, TileEntityManager.BUTTON_SIZE_H, x, y) &&
