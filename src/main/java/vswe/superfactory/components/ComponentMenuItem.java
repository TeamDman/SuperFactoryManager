...
public class ComponentMenuItem extends ComponentMenuStuff {
    ...
    protected void drawInfoMenuContent(GuiManager gui, int mX, int mY) {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            gui.drawTexture(x, y, ARROW_SRC_X + srcXArrow * ARROW_WIDTH, ARROW_SRC_Y + srcYArrow * ARROW_HEIGHT,
                    ARROW_WIDTH, ARROW_HEIGHT);
        }
        gui.drawCenteredString(getSelectedSetting().getFuzzyMode().toString(), ARROW_X_LEFT, ARROW_TEXT_Y, 0.7F,
                ARROW_X_RIGHT - ARROW_X_LEFT + ARROW_WIDTH, 0x404040);
    }
    ...
    public void onClick(int mX, int mY, int button) {
        ...
        if (isEditing()) {
             ...
             * getSelectedSetting().getItem().hasTagCompound()) {
             * getParent().getManager().specialRenderer = new
