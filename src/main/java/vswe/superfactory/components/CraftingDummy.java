...
public class CraftingDummy extends InventoryCrafting {
    ...
    public ItemStack getStackInSlot(int id) {
        if (overrideMap != null && !overrideMap.getOrDefault(id, ItemStack.EMPTY).isEmpty() &&
                overrideMap.get(id).getCount() > 0) {
            return overrideMap.get(id);
        } else {
            return id < 0 || id >= this.getSizeInventory() ? ItemStack.EMPTY :
    ...
    public boolean isItemValidForRecipe(IRecipe recipe, ItemSetting result, Map<Integer, ItemStack> overrideMap,
                                        boolean advanced) {
        this.overrideMap = overrideMap;
        if ((advanced && getRecipe() == null) ||
                (!advanced && !recipe.matches(this, crafting.getParent().getManager().getWorld()))) {
            return false;
        }
