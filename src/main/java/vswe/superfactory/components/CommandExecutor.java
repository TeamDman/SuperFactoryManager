...
public class CommandExecutor {
    ...
    private CommandExecutor(TileEntityManager manager, List<ItemBufferElement> itemBufferSplit,
                            List<CraftingBufferElement> craftingBufferHighSplit,
                            List<CraftingBufferElement> craftingBufferLowSplit,
                            List<FluidBufferElement> fluidBufferSplit, List<Integer> usedCommandCopy) {
        this.manager = manager;
    ...
    public static List<SlotInventoryHolder> getContainers(TileEntityManager manager, ComponentMenu componentMenu,
                                                          ConnectionBlockType type) {
                    ...
                    if (inventoryIndex == variableIndex) { // if we selected that variable
                        ...
                        for (int selected : selection) {
                            addContainer(inventories, ret, selected, menuContainer, type,
                                    ((ComponentMenuContainerTypes) variable.getDeclaration().getMenus().get(1))
                                            .getValidTypes());
                        }
                        break;
                    }
                ...
                for (BlockPos pos : positions) {
                    ...
                    if ((connection = inventoriesMap.getOrDefault(pos, null)) != null) {
                        if (connection.isOfType(type) && !connection.getTileEntity().isInvalid() &&
                                !containsTe(ret, connection.getTileEntity())) {
                            ret.add(new SlotInventoryHolder(baseIndex, connection.getTileEntity(),
                                    menuContainer.getOption()));
                        }
                    }
                }
            }
    ...
    private static void addContainer(List<ConnectionBlock> inventories, List<SlotInventoryHolder> ret, int selected,
                                     ComponentMenuContainer menuContainer, ConnectionBlockType requestType,
                                     EnumSet<ConnectionBlockType> variableType) {
        if (selected >= 0 && selected < inventories.size()) {
            ConnectionBlock connection = inventories.get(selected);

            if (connection.isOfType(requestType) && connection.isOfAnyType(variableType) &&
                    !connection.getTileEntity().isInvalid() && !containsTe(ret, connection.getTileEntity())) {
    ...
    private static boolean containsTe(List<SlotInventoryHolder> lst, TileEntity te) {
        for (SlotInventoryHolder slotInventoryHolder : lst) {
            if (slotInventoryHolder.getTile().getPos().equals(te.getPos()) &&
                    slotInventoryHolder.getTile().getClass().equals(te.getClass())) {
                return true;
            }
        }
        return false;
    ...
    public void executeTriggerCommand(FlowComponent command, EnumSet<ConnectionOption> validTriggerOutputs) {
        for (Variable variable : manager.getVariables()) {
            if (variable.isValid()) {
                if (!variable.hasBeenExecuted() ||
                        ((ComponentMenuVariable) variable.getDeclaration().getMenus().get(0)).getVariableMode() ==
                                ComponentMenuVariable.VariableMode.LOCAL) {
        ...
        try {
            ...
            switch (command.getType()) {
                ...
                case REDSTONE_EMITTER:
                    ...
                    if (emitters != null) {
                        for (SlotInventoryHolder emitter : emitters) {
                            emitter.getEmitter().updateState(
                                    (ComponentMenuRedstoneSidesEmitter) command.getMenus().get(1),
                                    (ComponentMenuRedstoneOutput) command.getMenus().get(2),
                                    (ComponentMenuPulse) command.getMenus().get(3));
                        }
                    }
                    break;
                ...
                case VARIABLE:
                    ...
                    if (tiles != null) {
                        updateVariable(tiles, (ComponentMenuVariable) command.getMenus().get(0),
                                (ComponentMenuListOrder) command.getMenus().get(3));
                    }
                    break;
                case FOR_EACH:
                    updateForLoop(command, (ComponentMenuVariableLoop) command.getMenus().get(0),
                            (ComponentMenuContainerTypes) command.getMenus().get(1),
                            (ComponentMenuListOrder) command.getMenus().get(2));
                    executeChildCommands(command, EnumSet.of(ConnectionOption.STANDARD_OUTPUT));
                    return;
                case AUTO_CRAFTING:
                    CraftingBufferElement element = new CraftingBufferElement(this,
                            (ComponentMenuCrafting) command.getMenus().get(0),
                            (ComponentMenuContainerScrap) command.getMenus().get(2));
                ...
                case GROUP:
                    if (connectionId < command.getChildrenInputNodes().size()) {
                        executeChildCommands(command.getChildrenInputNodes().get(connectionId),
                                EnumSet.allOf(ConnectionOption.class));
                    }
                    return;
                case NODE:
                        ...
                        for (int i = 0; i < parent.getChildrenOutputNodes().size(); i++) {
                            if (command.equals(parent.getChildrenOutputNodes().get(i))) {
                                Connection connection = parent
                                        .getConnection(parent.getConnectionSet().getInputCount() + i);
                                if (connection != null) {
                ...
                case CAMOUFLAGE:
                    ...
                    if (camouflage != null) {
                        ...
                        ComponentMenuCamouflageInside inside = (ComponentMenuCamouflageInside) command.getMenus()
                                .get(2);
                        ...
                        if (items.isFirstRadioButtonSelected() || items.getSettings().get(0).isValid()) {
                            ItemStack itemStack = items.isFirstRadioButtonSelected() ? ItemStack.EMPTY :
                                    ((ItemSetting) items.getSettings().get(0)).getItem();
    ...
    private void addItemToBuffer(ComponentMenuStuff menuItem, SlotInventoryHolder inventory, Setting setting,
                                 ItemStack itemStack, EnumFacing side, int slot) {
        if ((menuItem.useWhiteList() == (setting != null)) || (setting != null && setting.isLimitedByAmount())) {
            FlowComponent owner = menuItem.getParent();
            SlotStackInventoryHolder target = new SlotStackInventoryHolder(itemStack, inventory.getInventory(side),
                    slot);
            ...
            if (!added) {
                ItemBufferElement itemBufferElement = new ItemBufferElement(owner, setting, inventory,
                        menuItem.useWhiteList(), target);
                ...
                try {
                    ...
                    for (IFluidTankProperties fluidTankInfo : currentTankInfos) {
                        ...
                        for (IFluidTankProperties tankInfo : tankInfos) {
                            if (FluidStack.areFluidStackTagsEqual(tankInfo.getContents(),
                                    fluidTankInfo.getContents()) &&
                                    tankInfo.getCapacity() == fluidTankInfo.getCapacity()) {
                                alreadyUsed = true;
                            }
                        }
                        if (alreadyUsed) {
    ...
    private void addFluidToBuffer(ComponentMenuStuff menuItem, SlotInventoryHolder inventoryHolder, IFluidHandler tank,
                                  Setting setting, FluidStack fluidStack, EnumFacing side) {
        if ((menuItem.useWhiteList() == (setting != null)) || (setting != null && setting.isLimitedByAmount())) {
            FlowComponent owner = menuItem.getParent();
            ...
            if (!added) {
                FluidBufferElement itemBufferElement = new FluidBufferElement(owner, setting, inventoryHolder,
                        menuItem.useWhiteList(), target);
    ...
    private void insertItems(ComponentMenu componentMenu, List<SlotInventoryHolder> inventories) {
        ...
        for (SlotInventoryHolder inventoryHolder : inventories) {
            ...
            for (CraftingBufferElement craftingBufferElement : craftingBufferHigh) {
                insertItemsFromInputBufferElement(menuItem, inventories, outputCounters, inventoryHolder,
                        craftingBufferElement);
            }
            for (ItemBufferElement itemBufferElement : itemBuffer) {
                insertItemsFromInputBufferElement(menuItem, inventories, outputCounters, inventoryHolder,
                        itemBufferElement);
    ...
    private void insertItemsFromInputBufferElement(ComponentMenuStuff menuItem, List<SlotInventoryHolder> inventories,
                                                   List<OutputItemCounter> outputCounters,
                                                   SlotInventoryHolder inventoryHolder,
                                                   IItemBufferElement itemBufferElement) {
        ...
        while ((subElement = itemBufferElement.getSubElement()) != null &&
                (remaining = subElement.getSizeRemaining()) > 0) {
            ...
            OutputItemCounter outputItemCounter = outputCounters.stream()
                    ...
                    .findFirst()
                    .orElseGet(() -> {
                        OutputItemCounter c = new OutputItemCounter(itemBuffer, inventories, inventoryHolder, setting,
                                menuItem.useWhiteList());
            ...
            for (FluidBufferElement fluidBufferElement : fluidBuffer) {
                ...
                while (fluidIterator.hasNext()) {
                    ...
                    if ((menuItem.useWhiteList() == (setting == null)) &&
                            (setting == null || !setting.isLimitedByAmount())) {
                        continue;
                    }
                    ...
                    if (outputFluidCounter == null) {
                        outputFluidCounter = new OutputFluidCounter(fluidBuffer, tanks, tankHolder, setting,
                                menuItem.useWhiteList());
    ...
    private boolean searchForStuff(ComponentMenu componentMenu, List<SlotInventoryHolder> inventories,
                                   boolean useFluids) {
    ...
    private void calculateConditionData(ComponentMenu componentMenu, SlotInventoryHolder inventoryHolder,
    ...
    private void calculateConditionDataItem(ComponentMenu componentMenu, SlotInventoryHolder inventoryHolder,
                                            Map<Integer, ConditionSettingChecker> conditionSettingCheckerMap) {
        IdentityHashMap<ItemStack, Object> seenStacks = new IdentityHashMap<>();
        for (SideSlotTarget sideSlotTarget : inventoryHolder.getValidSlots().values()) {
            for (int slot : sideSlotTarget.getSlots()) {
                ...
                if (setting != null) {
                    ...
                    if (conditionSettingChecker == null) {
                        conditionSettingCheckerMap.put(setting.getId(),
    ...
    private void calculateConditionDataFluid(ComponentMenu componentMenu, SlotInventoryHolder inventoryHolder,
                                             Map<Integer, ConditionSettingChecker> conditionSettingCheckerMap) {
        for (SideSlotTarget sideSlotTarget : inventoryHolder.getValidSlots().values()) {
            ...
            for (IFluidTankProperties fluidTankInfo : currentTankInfos) {
                ...
                boolean alreadyUsed = false;
                for (IFluidTankProperties tankInfo : tankInfos) {
                    if (FluidStack.areFluidStackTagsEqual(tankInfo.getContents(), fluidTankInfo.getContents()) &&
                            tankInfo.getCapacity() == fluidTankInfo.getCapacity()) {
                        alreadyUsed = true;
                    }
                }
    ...
    private boolean checkConditionResult(ComponentMenu componentMenu,
                                         Map<Integer, ConditionSettingChecker> conditionSettingCheckerMap) {
        ComponentMenuStuff menuItem = (ComponentMenuStuff) componentMenu;
        ...
        if (split.useSplit()) {
            ...
            for (int i = 0; i < connections.length; i++) {
                ...
                if (!connectionOption.isInput() && connection != null) {
                    ...
                    CommandExecutor newExecutor = new CommandExecutor(manager, itemBufferSplit,
                            new ArrayList<CraftingBufferElement>(craftingBufferHigh),
                            new ArrayList<CraftingBufferElement>(craftingBufferLow), fluidBufferSplit, usedCommandCopy);
    ...
    private void updateVariable(List<SlotInventoryHolder> tiles, ComponentMenuVariable menuVariable,
                                ComponentMenuListOrder menuOrder) {
        ...
        if (variable.isValid()) {
            ...
            List<ConnectionBlock> inventories = manager.getConnectedInventories();
            EnumSet<ConnectionBlockType> validTypes = ((ComponentMenuContainerTypes) variable.getDeclaration()
    ...
    private void updateForLoop(FlowComponent command, ComponentMenuVariableLoop variableMenu,
                               ComponentMenuContainerTypes typesMenu, ComponentMenuListOrder orderMenu) {
        ...
        for (Integer selected : selection) {
            ...
            if (selected >= 0 && selected < inventories.size()) {
                ...
                if (inventory.isOfAnyType(validTypes)) {
                    // Store context
                    final List<CraftingBufferElement> craftingBufferHigh = ImmutableList
                            .copyOf(this.craftingBufferHigh);
