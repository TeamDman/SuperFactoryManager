...
public abstract class ComponentMenuContainer extends ComponentMenu {
    ...
    public ComponentMenuContainer(FlowComponent parent, ConnectionBlockType validType) {
        ...
        scrollController = new ScrollController<IContainerSelection>(getDefaultSearch()) {
            ...
            protected void onClick(IContainerSelection iContainerSelection, int mX, int mY, int button) {
                if (GuiScreen.isShiftKeyDown() && mX != -1 && mY != -1) {
                    ...
                    setSelectedInventoryAndSync(iContainerSelection.getId(),
            ...
            public void drawMouseOver(GuiManager gui, int mX, int mY) {
                if (locked && GuiBase.isShiftKeyDown()) {
                    ...
                    cachedTooltip.drawMouseOverMouseOver(gui,
                            lockedX + gui.getAdvancedToolTipContentStartX(cachedTooltip),
                            lockedY + gui.getAdvancedToolTipContentStartY(cachedTooltip), mX, mY);
                } else {
                    locked = false;
                    keepCache = false;
            ...
            class ToolTip implements IAdvancedTooltip {
                ...
                private void drawBlock(GuiBase gui, int x, int y, int mX, int mY, EnumFacing direction) {
                    ...
                    gui.drawTexture(x, y, SRC_X, SRC_Y + (CollisionHelper.inBounds(x, y, 16, 16, mX, mY) ? 16 : 0), 16,
                            16);
                ...
                private void drawMouseOverMouseOver(GuiBase gui, int x, int y, int mX, int mY) {
                    boolean ignored = drawBlockMouseOver(gui, x + 25, y + 5, mX, mY, EnumFacing.NORTH) ||
        ...
        buttons.add(new PageButton(Localization.FILTER_SHORT, Page.MAIN, Localization.FILTER_LONG, Page.FILTER, false,
                102, 21));
        buttons.add(new PageButton(Localization.MULTI_SHORT, Page.MAIN, Localization.MULTI_LONG, Page.MULTI, false, 111,
                21));
        ...
        for (int i = 0; i < subFilterPages.length; i++) {
            buttons.add(new ComponentMenuContainer.PageButton(Localization.SUB_MENU_SHORT,
                    ComponentMenuContainer.Page.FILTER, Localization.SUB_MENU_LONG, subFilterPages[i], true,
                    FILTER_BUTTON_X, CHECK_BOX_FILTER_Y + CHECK_BOX_FILTER_SPACING * i + FILTER_BUTTON_Y));
        }
        buttons.add(new ComponentMenuContainer.Button(Localization.CLEAR_SHORT, ComponentMenuContainer.Page.FILTER,
                Localization.CLEAR_LONG, true, FILTER_RESET_BUTTON_X, CHECK_BOX_FILTER_INVERT_Y) {
        ...
        buttons.add(new Button(Localization.SELECT_INVERT_SHORT, Page.MAIN, Localization.SELECT_INVERT_LONG, false, 102,
                60) {
    ...
    void drawContainer(GuiManager gui, IContainerSelection iContainerSelection, List<Integer> selected, int x, int y,
        ...
        gui.drawTexture(x, y, INVENTORY_SRC_X + srcInventoryX * INVENTORY_SIZE,
                INVENTORY_SRC_Y + srcInventoryY * INVENTORY_SIZE, INVENTORY_SIZE, INVENTORY_SIZE);
    ...
    public void draw(GuiManager gui, int mX, int mY) {
        ...
        if (currentPage == Page.MAIN) {
            ...
            gui.drawCenteredString(selectedInventories.size() + " " + Localization.SELECTED_CONTAINERS.toString(),
                    TEXT_MULTI_MARGIN_X, TEXT_MULTI_Y, 0.9F, MENU_WIDTH - TEXT_MULTI_MARGIN_X * 2, 0x404040);
            String error = null;
            ...
            if (error != null) {
                gui.drawSplitString(error, TEXT_MULTI_MARGIN_X, TEXT_MULTI_ERROR_Y,
                        MENU_WIDTH - TEXT_MULTI_MARGIN_X * 2, 0.7F, 0x404040);
            }
        ...
        if (currentPage == Page.MAIN) {
            ...
            if (CollisionHelper.inBounds(5, 60, MENU_WIDTH - 20, 5, mX, mY)) {
                ...
                str += "\n" + Localization.X.toString() + " (" +
                        (filter.lowerRange[0].getNumber() + getParent().getManager().getPos().getX()) + ", " +
                        (filter.higherRange[0].getNumber() + getParent().getManager().getPos().getX()) + ")";
                str += "\n" + Localization.Y.toString() + " (" +
                        (filter.lowerRange[1].getNumber() + getParent().getManager().getPos().getY()) + ", " +
                        (filter.higherRange[1].getNumber() + getParent().getManager().getPos().getY()) + ")";
                str += "\n" + Localization.Z.toString() + " (" +
                        (filter.lowerRange[2].getNumber() + getParent().getManager().getPos().getZ()) + ", " +
                        (filter.higherRange[2].getNumber() + getParent().getManager().getPos().getZ()) + ")";
