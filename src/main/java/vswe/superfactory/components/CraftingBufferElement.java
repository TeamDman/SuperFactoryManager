...
public class CraftingBufferElement implements IItemBufferElement, IItemBufferSubElement {
    ...
    private void disposeOfExtraItem(ItemStack itemStack) {
        ...
        List<SlotInventoryHolder> inventories = CommandExecutor.getContainers(manager, scrapMenu,
                ConnectionBlockType.INVENTORY);
            ...
            for (SideSlotTarget sideSlotTarget : targets) {
                ...
                for (int slot : sideSlotTarget.getSlots()) {
                    if (inventory.insertItem(slot, itemStack, true) != itemStack) {
                        ...
                        if (itemInSlot.isEmpty() || (itemInSlot.isItemEqual(itemStack) &&
                                ItemStack.areItemStackTagsEqual(itemStack, itemInSlot) && itemStack.isStackable())) {
                            int itemCountInSlot = itemInSlot.isEmpty() ? 0 : itemInSlot.getCount();

                            int moveCount = Math.min(itemStack.getCount(),
                                    Math.min(inventory.getSlotLimit(slot), itemStack.getMaxStackSize()) -
                                            itemCountInSlot);
    ...
    private boolean findItems(boolean remove) {
        ...
        for (ItemBufferElement itemBufferElement : executor.itemBuffer) {
            ...
            for (Iterator<SlotStackInventoryHolder> iterator = itemBufferElement.getSubElements().iterator(); iterator
                    .hasNext();) {
                ...
                for (int i = 0; i < 9; i++) {
                    ...
                    if (foundItems.getOrDefault(i, ItemStack.EMPTY).isEmpty()) {
                        if (!setting.isValid()) {
                            ...
                            if (craftingMenu.getDummy().isItemValidForRecipe(recipe, craftingMenu.getResultItem(),
                                    foundItems, useAdvancedDetection())) {
                                subCount--;
                                count--;
                                if (remove) {
