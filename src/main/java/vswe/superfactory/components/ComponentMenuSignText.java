...
public class ComponentMenuSignText extends ComponentMenu {
    ...
    public ComponentMenuSignText(FlowComponent parent) {
        ...
        for (int i = 0; i < textBoxes.length; i++) {
            ...
            checkBoxes.addCheckBox(new CheckBox(Localization.UPDATE_LINE, CHECK_BOX_X,
                    CHECK_BOX_Y + TEXT_BOX_Y + i * TEXT_BOX_Y_SPACING) {
    ...
    public void draw(GuiManager gui, int mX, int mY) {
        for (int i = 0; i < textBoxes.length; i++) {
            ...
            gui.drawTexture(TEXT_BOX_X, y, TEXT_BOX_SRC_X, TEXT_BOX_SRC_Y + srcBoxY * TEXT_BOX_SIZE_H, TEXT_BOX_SIZE_W,
                    TEXT_BOX_SIZE_H);
            gui.drawString(textBox.getText(), TEXT_BOX_X + TEXT_BOX_TEXT_X, y + TEXT_BOX_TEXT_Y, 0.6F, 0xFFFFFF);

            if (selected == i) {
                gui.drawCursor(TEXT_BOX_X + textBox.getCursorPosition(gui) + CURSOR_X, y + CURSOR_Y, CURSOR_Z,
                        0xFFFFFFFF);
            }
        }
    ...
    public void onClick(int mX, int mY, int button) {
        for (int i = 0; i < textBoxes.length; i++) {
            if (CollisionHelper.inBounds(TEXT_BOX_X, TEXT_BOX_Y + TEXT_BOX_Y_SPACING * i, TEXT_BOX_SIZE_W,
