...
public abstract class ScrollController<T> {
    ...
    public void draw(GuiManager gui, int mX, int mY) {
        ...
        if (hasSearchBox) {
            gui.drawTexture(TEXT_BOX_X, TEXT_BOX_Y, TEXT_BOX_SRC_X, TEXT_BOX_SRC_Y + srcBoxY * TEXT_BOX_SIZE_H,
                    TEXT_BOX_SIZE_W, TEXT_BOX_SIZE_H);
            gui.drawString(textBox.getDisplayText(), TEXT_BOX_X + TEXT_BOX_TEXT_X, TEXT_BOX_Y + TEXT_BOX_TEXT_Y,
                    0xFFFFFF);

            if (selected) {
                gui.drawCursor(TEXT_BOX_X + textBox.getCursorPosition(gui) + CURSOR_X, TEXT_BOX_Y + CURSOR_Y, CURSOR_Z,
                        0xFFFFFFFF);
            }

            if (textBox.getText().length() > 0 || result.size() > 0) {
                gui.drawString(Localization.ITEMS_FOUND.toString() + " " + result.size(), AMOUNT_TEXT_X, AMOUNT_TEXT_Y,
                        0.7F, 0x404040);
            }
        }
        ...
        if (result.size() > 0) {
            ...
            List<Point> points = getItemCoordinates();
            for (Point point : points) {
                draw(gui, result.get(point.id), point.x, point.y,
    ...
    private void drawArrow(GuiManager gui, boolean down, int mX, int mY) {
        if (canScroll) {
            ...
            gui.drawTexture(ARROW_X, down ? ARROW_Y_DOWN : ARROW_Y_UP, ARROW_SRC_X + srcArrowX * ARROW_SIZE_W,
                    ARROW_SRC_Y + srcArrowY * ARROW_SIZE_H, ARROW_SIZE_W, ARROW_SIZE_H);
        }
    }
