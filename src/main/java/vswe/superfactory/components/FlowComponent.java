...
public class FlowComponent implements IComponentNetworkReader, Comparable<FlowComponent> {
    ...
    public static FlowComponent readFromNBT(TileEntityManager jam, NBTTagCompound nbtTagCompound, int version,
        ...
        try {
            ...
            for (int i = 0; i < connections.tagCount(); i++) {
                ...
                Connection connection = new Connection(componentId,
    ...
    public void draw(GuiManager gui, int mX, int mY, int zLevel) {
        ...
        gui.drawTexture(x, y, isLarge ? COMPONENT_SRC_LARGE_X : COMPONENT_SRC_X, COMPONENT_SRC_Y, getComponentWidth(),
                getComponentHeight());
        ...
        gui.drawTexture(x + getComponentWidth() + ARROW_X, y + ARROW_Y, ARROW_SRC_X + ARROW_SIZE_W * srcArrowX,
                ARROW_SRC_Y + ARROW_SIZE_H * srcArrowY, ARROW_SIZE_W, ARROW_SIZE_H);
        ...
        if (isLarge) {
            for (int i = 0; i < menus.size(); i++) {
                ...
                gui.drawTexture(itemX + MENU_ARROW_X, itemY + MENU_ARROW_Y,
                        MENU_ARROW_SRC_X + MENU_ARROW_SIZE_W * srcItemArrowX,
                        MENU_ARROW_SRC_Y + MENU_ARROW_SIZE_H * srcItemArrowY, MENU_ARROW_SIZE_W, MENU_ARROW_SIZE_H);

                gui.drawString(menu.getName(), x + MENU_X + MENU_ITEM_TEXT_X, y + getMenuItemY(i) + MENU_ITEM_TEXT_Y,
                        0x404040);
        ...
        for (int i = 0; i < connectionSet.getConnections().length; i++) {
            ...
            int srcConnectionX = (CollisionHelper.inBounds(location[0], location[1], connectionWidth, connectionHeight,
                    mX, mY)) ? 1 : 0;

            Connection current = manager.getCurrentlyConnecting();
            if (current != null && current.getComponentId() == id && current.getConnectionId() == i) {
                gui.drawLine(location[0] + connectionWidth / 2, location[1] + connectionHeight / 2,
                        overrideX != -1 ? overrideX : mX, overrideY != -1 ? overrideY : mY);
            }
            ...
            gui.drawTexture(location[0], location[1], CONNECTION_SRC_X + srcConnectionX * connectionWidth, location[2],
                    connectionWidth, connectionHeight);
        ...
        if (!errors.isEmpty()) {
            int srcErrorY = CollisionHelper.inBounds(x + ERROR_X, y + ERROR_Y, ERROR_SIZE_W, ERROR_SIZE_H, mX, mY) ? 1 :
                    0;
            gui.drawTexture(x + ERROR_X, y + ERROR_Y, ERROR_SRC_X, ERROR_SRC_Y + srcErrorY * ERROR_SIZE_H, ERROR_SIZE_W,
                    ERROR_SIZE_H);
        }
        ...
        if (name != null && Settings.isCommandTypes() && !GuiScreen.isCtrlKeyDown()) {
            gui.drawCenteredString(getType().getName(), x + DRAGGABLE_SIZE, y + 3, 0.7F,
                    getComponentWidth() - DRAGGABLE_SIZE - ARROW_SIZE_W, 0x707070);
        }

        if (isLarge) {
            if (isEditing) {
                gui.drawCursor(x + TEXT_X + (int) ((textBox.getCursorPosition(gui) + CURSOR_X) * 0.7F),
                        y + TEXT_Y + (int) (CURSOR_Y * 0.7F), CURSOR_Z, 0.7F, 0xFFFFFFFF);
                for (int i = 0; i < 2; i++) {
                    int buttonX = x + EDIT_X_SMALL;
                    int buttonY = y + (i == 0 ? EDIT_Y_TOP : EDIT_Y_BOT);

                    int srcXButton = CollisionHelper.inBounds(buttonX, buttonY, EDIT_SIZE_SMALL, EDIT_SIZE_SMALL, mX,
                            mY) ? 1 : 0;
                    int srcYButton = i;

                    gui.drawTexture(buttonX, buttonY, EDIT_SRC_X + srcXButton * EDIT_SIZE_SMALL,
                            EDIT_SRC_Y + EDIT_SIZE + EDIT_SIZE_SMALL * srcYButton, EDIT_SIZE_SMALL, EDIT_SIZE_SMALL);
                }
            } else {
                ...
                gui.drawTexture(buttonX, buttonY, EDIT_SRC_X + srcXButton * EDIT_SIZE, EDIT_SRC_Y, EDIT_SIZE,
                        EDIT_SIZE);
            }
        }
    ...
    public void drawMouseOver(GuiManager gui, int mX, int mY) {
        ...
        for (ConnectionOption connection : connectionSet.getConnections()) {
            ...
            if (CollisionHelper.inBounds(location[0], location[1], CONNECTION_SIZE_W, CONNECTION_SIZE_H, mX, mY)) {
                gui.drawMouseOver(connection.getName(this, (connection.isInput() ? inputCount : outputCount) - 1), mX,
                        mY);
            }
        }
    ...
    public boolean onClick(int mX, int mY, int button) {
        if (CollisionHelper.inBounds(x, y, getComponentWidth(), getComponentHeight(), mX, mY)) {
            ...
            if (internalX <= DRAGGABLE_SIZE && internalY <= DRAGGABLE_SIZE) {
                ...
                if (isLarge && isEditing && CollisionHelper.inBounds(EDIT_X_SMALL, EDIT_Y_TOP, EDIT_SIZE_SMALL,
                        EDIT_SIZE_SMALL, internalX, internalY)) {
                            isEditing = false;
                    ...
                    if (isLarge && isEditing && CollisionHelper.inBounds(EDIT_X_SMALL, EDIT_Y_BOT, EDIT_SIZE_SMALL,
                            EDIT_SIZE_SMALL, internalX, internalY)) {
                                isEditing = false;
                                textBox.setText(null);
                            } else
                        if (isLarge && isEditing && CollisionHelper.inBounds(TEXT_X, TEXT_Y, TEXT_SPACE, TEXT_HEIGHT,
                                internalX, internalY)) {
                                    if (button == 1) {
                            ...
                            if (inArrowBounds(internalX, internalY) ||
                                    (Settings.isLargeOpenHitBox() && internalY < COMPONENT_SIZE_H)) {
                                        if (!isLarge && type == ComponentType.GROUP && Settings.isQuickGroupOpen() &&
                ...
                if (CollisionHelper.inBounds(location[0], location[1], CONNECTION_SIZE_W, CONNECTION_SIZE_H, mX, mY)) {
                    ...
                    if (button == 1 && current == null) {
                        ...
                        if (current == null) {
                            ...
                            if (manager.getFlowItems().size() <= current.getComponentId() ||
                                    current.getComponentId() < 0)
                                return true;

                            FlowComponent connectTo = manager.getFlowItems().get(current.getComponentId());

                            if (connectTo.connectionSet.getConnections().length <= current.getConnectionId() ||
                                    current.getConnectionId() < 0)
                                return true;
            ...
            if (!currentComponent.connectionSet.getConnections()[i].isInput()) {
                ...
                if (c != null) {
                    if (c.getComponentId() >= 0 && c.getComponentId() < manager.getFlowItems().size()) {
                        ...
                        if (checkForLoops(usedComponentsCopy, manager.getFlowItems().get(c.getComponentId()),
    ...
    private boolean inArrowBounds(int internalX, int internalY) {
        return CollisionHelper.inBounds(getComponentWidth() + ARROW_X, ARROW_Y, ARROW_SIZE_W, ARROW_SIZE_H, internalX,
                internalY);
    }

    private boolean inMenuArrowBounds(int i, int internalX, int internalY) {
        return CollisionHelper.inBounds(MENU_X + MENU_ARROW_X, getMenuItemY(i) + MENU_ARROW_Y, MENU_ARROW_SIZE_W,
                MENU_ARROW_SIZE_H, internalX, internalY);
    }
                    ...
                    if (deleted) {
                        ...
                        if (created) {
                            ...
                            if (connection.getNodes().size() < MAX_NODES &&
                                    (!manager.getWorld().isRemote || length > connection.getNodes().size())) {
    ...
    public void refreshData(ContainerManager container, FlowComponent newData) {
        ...
        for (int i = 0; i < connectionSet.getConnections().length; i++) {
            ...
            if (newData.connections.get(i) != null && (connections.get(i) == null ||
                ...
                DataWriter dw = PacketHandler.getWriterForClientComponentPacket(container, this, null);
                writeConnectionData(dw, i, true, connections.get(i).getComponentId(),
                ...
                if (deleted) {
                    ...
                    for (int j = 0; j < newConnection.getNodes().size(); j++) {
                        ...
                        if (node.getX() != newNode.getX() || node.getY() != newNode.getY()) {
                            sendClientConnectionNode(container, newConnection.getNodes().size(), i, j, true, false, 0,
                                    0);
                            hasDeleted = true;
                            break;
                        }
                    ...
                    if (!hasDeleted) {
                        sendClientConnectionNode(container, newConnection.getNodes().size(), i,
                                newConnection.getNodes().size(), true, false, 0, 0);
                    }
                } else {
                        ...
                        if (node.getX() != newNode.getX() || node.getY() != newNode.getY()) {
                            ...
                            if (created) {
                                ...
                                if (node.getX() == nextNode.getX() && node.getY() == nextNode.getY()) {
                                    sendClientConnectionNode(container, newConnection.getNodes().size(), i, j, false,
                                            true, newNode.getX(), newNode.getY());
                                    break;
                                }
                            }
                            sendClientConnectionNode(container, newConnection.getNodes().size(), i, j, false, false,
                    ...
                    if (!updated && created) {
                        ...
                        sendClientConnectionNode(container, newConnection.getNodes().size(), i, nodeId, false, true,
