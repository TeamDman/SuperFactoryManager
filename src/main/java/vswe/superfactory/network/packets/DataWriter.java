...
public class DataWriter {
    ...
    public void writeComponentId(TileEntityManager manager, int id) {
        if (!idWritten) {
            if (Settings.isLimitless(manager) &&
                    manager.getFlowItems().size() > TileEntityManager.MAX_COMPONENT_AMOUNT) {
                writeBoolean(true);
    ...
    public void writeInventoryId(TileEntityManager manager, int id) {
        ...
        if (!invWritten) {
            manager.updateFirst();
            if (Settings.isLimitless(manager) &&
                    manager.getConnectedInventories().size() > TileEntityManager.MAX_CONNECTED_INVENTORIES) {
                writeBoolean(true);
