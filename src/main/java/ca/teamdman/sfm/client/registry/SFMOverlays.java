...
public class SFMOverlays {
    ...
    public static void onRegisterOverlays(RenderGameOverlayEvent.Post event) {
        if (event.getType() == RenderGameOverlayEvent.ElementType.HOTBAR) {
            LABEL_GUN_REMINDER_OVERLAY.render(event.getPartialTicks(), event.getResolution().getScaledWidth(),
                    event.getResolution().getScaledHeight());
            NETWORK_TOOL_REMINDER_OVERLAY.render(event.getPartialTicks(), event.getResolution().getScaledWidth(),
                    event.getResolution().getScaledHeight());
        }
    }
}
