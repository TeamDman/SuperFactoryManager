...
public class LibraryWithStructIOProtocolsGameTest extends SFMGameTestDefinition {
    ...
    public void run(SFMGameTestHelper helper) {
        ...
        helper.setBlock(devicePos, SFMBlocks.TEST_BARREL_BLOCK.get());
        ...
        helper.runAfterDelay(10, () -> {
            ...
            helper.succeedIfManagerDidThingWithoutLagging(manager, () -> {
                ...
                assertTrue(
                        count(sourceHandler, Blocks.DIRT) == 0,
                        "Source should be empty but has " + count(sourceHandler, Blocks.DIRT) + " dirt"
                );
                assertTrue(
                        count(deviceHandler, Blocks.DIRT) == 32,
                        "Device should have 32 dirt in input slots but has " + count(deviceHandler, Blocks.DIRT)
                );

                // Verify items were moved from device's output slots to dest
                var destHandler = helper.getItemHandler(destPos);
                assertTrue(
                        count(destHandler, Blocks.STONE) == 16,
                        "Dest should have 16 stone but has " + count(destHandler, Blocks.STONE)
                );
            });
        });
    }
