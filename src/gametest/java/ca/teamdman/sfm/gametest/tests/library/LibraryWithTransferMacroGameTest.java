...
public class LibraryWithTransferMacroGameTest extends SFMGameTestDefinition {
    ...
    public void run(SFMGameTestHelper helper) {
        ...
        // Place main blocks
        helper.setBlock(inputPos, SFMBlocks.TEST_BARREL_BLOCK.get());
        helper.setBlock(libraryPos, SFMBlocks.LIBRARY_BLOCK.get());
        helper.setBlock(managerPos, SFMBlocks.MANAGER_BLOCK.get());
        helper.setBlock(processorPos, SFMBlocks.TEST_BARREL_BLOCK.get());
        helper.setBlock(outputPos, SFMBlocks.TEST_BARREL_BLOCK.get());
        ...
        ItemStack libraryDisk = new ItemStack(SFMItems.DISK_ITEM.get());
        ...
        helper.runAfterDelay(10, () -> {
            ...
            helper.succeedIfManagerDidThingWithoutLagging(manager, () -> {
                ...
                assertTrue(
                        count(inputHandler, Blocks.IRON_ORE) == 0,
                        "Input should be empty but has " + count(inputHandler, Blocks.IRON_ORE) + " iron ore"
                );
                assertTrue(
                        count(processorHandler, Blocks.IRON_ORE) == 64,
                        "Processor should have 64 iron ore in input slots but has " + count(processorHandler, Blocks.IRON_ORE)
                );

                // Verify items were moved from processor's output slots to output
                var outputHandler = helper.getItemHandler(outputPos);
                assertTrue(
                        count(outputHandler, Blocks.IRON_BLOCK) == 8,
                        "Output should have 8 iron blocks but has " + count(outputHandler, Blocks.IRON_BLOCK)
                );
                assertTrue(
                        count(processorHandler, Blocks.IRON_BLOCK) == 0,
                        "Processor output slots should be empty but has " + count(processorHandler, Blocks.IRON_BLOCK)
                );
            });
        });
    }
