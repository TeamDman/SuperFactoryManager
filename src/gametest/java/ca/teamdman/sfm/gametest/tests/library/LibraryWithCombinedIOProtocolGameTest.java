...
public class LibraryWithCombinedIOProtocolGameTest extends SFMGameTestDefinition {
    ...
    public void run(SFMGameTestHelper helper) {
        ...
        helper.setBlock(machinePos, SFMBlocks.TEST_BARREL_BLOCK.get());
        helper.setBlock(destPos, SFMBlocks.TEST_BARREL_BLOCK.get());
        ...
        helper.runAfterDelay(10, () -> {
            ...
            helper.succeedIfManagerDidThingWithoutLagging(manager, () -> {
                ...
                assertTrue(
                        count(sourceHandler, Blocks.COAL_ORE) == 0,
                        "Source should be empty but has " + count(sourceHandler, Blocks.COAL_ORE) + " coal ore"
                );
                assertTrue(
                        count(machineHandler, Blocks.COAL_ORE) == 32,
                        "Machine should have 32 coal ore in input slots but has " + count(machineHandler, Blocks.COAL_ORE)
                );

                // Verify items were moved from machine's output slots to dest
                var destHandler = helper.getItemHandler(destPos);
                assertTrue(
                        count(destHandler, Blocks.COAL_BLOCK) == 4,
                        "Dest should have 4 coal blocks but has " + count(destHandler, Blocks.COAL_BLOCK)
                );
                assertTrue(
                        count(machineHandler, Blocks.COAL_BLOCK) == 0,
                        "Machine output slots should be empty but has " + count(machineHandler, Blocks.COAL_BLOCK)
                );
            });
        });
    }
